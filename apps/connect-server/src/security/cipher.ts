export class WydCipher {
  private static readonly keys = new Uint8Array([
  ...
  static decrypt(inputBuffer: Buffer): Buffer | null {
    ...
    for (let i = 4, iterator = 0; i < length; i++, keyIncrement++) {
    ...
    const calculatedChecksum = (checksumEnc - checksumDec) & 0xff;
  ...
  static encrypt(buffer: Buffer): Buffer {
    ...
    let keyIncrement = this.keys[hashKey * 2] & 0xff;
    const length = buffer.readUInt16LE(0);
